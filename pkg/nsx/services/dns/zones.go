...
// ValidateEndpointsByZone maps each endpoint to a zone and row; returns rows or err.
func (s *DNSRecordService) ValidateEndpointsByZone(namespace string, owner *ResourceRef, eps []*extdns.Endpoint) ([]EndpointRow, error) {
	if s.VPCService == nil {
...
func (s *DNSRecordService) SyncDNSZonesByVpcNetworkConfig(vpcConfig *v1alpha1.VPCNetworkConfiguration) (map[string]string, error) {
	...
	for _, p := range dnsZonePaths {
		...
		s.DNSZoneMap[p] = domain
		dnsZoneDomainMapping[p] = domain
	}
	return dnsZoneDomainMapping, nil
}

// DomainNamesInSpecOrder returns delegated domain names in the same order as specPaths (VPCNetworkConfiguration.Spec.DNSZones).
func DomainNamesInSpecOrder(pathToDomain map[string]string, specPaths []string) []string {
	if len(specPaths) == 0 || len(pathToDomain) == 0 {
		return nil
	}
	out := make([]string, 0, len(specPaths))
	for _, p := range specPaths {
		if d, ok := pathToDomain[p]; ok && d != "" {
			out = append(out, d)
		}
	}
	if len(out) == 0 {
