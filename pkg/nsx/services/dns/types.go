...
type DNSRecordProvider interface {
	CreateOrUpdateRecords(ctx context.Context, batch *AggregatedDNSEndponts) (bool, error)
	DeleteRecordByOwnerNN(ctx context.Context, kind, namespace, name string) (bool, error)
	ValidateEndpointsByZone(namespace string, owner *ResourceRef, eps []*extdns.Endpoint) ([]EndpointRow, error)
