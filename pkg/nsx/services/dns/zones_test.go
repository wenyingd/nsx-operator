...
func TestDomainNamesInSpecOrder(t *testing.T) {
	require.Nil(t, DomainNamesInSpecOrder(nil, []string{"/z"}))
	require.Nil(t, DomainNamesInSpecOrder(map[string]string{"/z": "a.com"}, nil))

	m := map[string]string{"/zones/a": "a.example.com", "/zones/b": "b.example.com"}
	require.Equal(t, []string{"b.example.com", "a.example.com"}, DomainNamesInSpecOrder(m, []string{"/zones/b", "/zones/a"}))
	require.Equal(t, []string{"a.example.com"}, DomainNamesInSpecOrder(m, []string{"/zones/a", "/zones/missing"}))
}
