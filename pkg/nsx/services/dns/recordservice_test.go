...
func requireNoErrDeleteDNS(ctx context.Context, t *testing.T, svc *DNSRecordService, kind, ns, name string) {
	t.Helper()
	_, err := svc.DeleteRecordByOwnerNN(ctx, kind, ns, name)
	require.NoError(t, err)
}
...
func TestValidateEndpointsByZone_table(t *testing.T) {
	...
	tableOwner := func(ns string) *ResourceRef {
		return &ResourceRef{
			Kind:   ResourceKindGateway,
			Object: &metav1.ObjectMeta{Namespace: ns, Name: "gw-table", UID: types.UID("zone-table")},
		}
	}
	...
	tests := []struct {
		...
		},
		{
			...
			ns:     "n", eps: []*extdns.Endpoint{ep}, errSub: "failed to find VPC Networkconfigurations",
		},
		{
	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T, env *testDNSSvc) {
				...
				requireNoErrDeleteDNS(ctx, t, env.DNSRecordService, ResourceKindGateway, "app", "gw1")
...
func TestCreateOrUpdateRecords_ownerScoped_mergeUpdatesTargets(t *testing.T) {
	...
	rec := store.GetByKey(recordStoreKey(testDNSZonePathZ, recordName, string(extdns.RecordTypeA)))
...
func Test_CreateOrUpdateRecords_Service_DeleteByOwnerNN(t *testing.T) {
	...
	requireNoErrDeleteDNS(ctx, t, env.DNSRecordService, ResourceKindService, "ns1", "lbsvc")
...
func TestContributingHelpers_table(t *testing.T) {
	...
	t.Run("replaceContributingOwnersInTags", func(t *testing.T) {
		tags := []model.Tag{
			modelTag(servicecommon.TagScopeDNSRecordContributingOwners, "old"),
		...
		require.ElementsMatch(t, []model.Tag{
			modelTag(servicecommon.TagScopeCluster, "c"),
			modelTag(servicecommon.TagScopeDNSRecordContributingOwners, "k1,k2"),
		}, out)
	})
}
...
func TestAppendRecordOwnershipTags_table(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := appendRecordOwnershipTags(in, tc.gwKey, tc.contributingKeys)
