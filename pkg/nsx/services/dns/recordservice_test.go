...
// newTestDNSRecordServiceForZoneValidate is a service that can run ValidateEndpointsByZone (VPC, DNS zone map, store).
...
func requireNoErrCreateDNS(ctx context.Context, t *testing.T, svc *DNSRecordService, batch *AggregatedDNSEndponts) {
	t.Helper()
	_, err := svc.CreateOrUpdateRecords(ctx, batch)
...
func TestCreateOrUpdateRecords_edgeCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := tt.svc.CreateOrUpdateRecords(ctx, tt.batch)
			if tt.errSubstring != "" {
				require.Error(t, err)
...
func TestValidateEndpointsByZone_FQDN_takenInZone_differentTargetValues(t *testing.T) {
	ctx := context.Background()
	...
	_, err := svc.ValidateEndpointsByZone("ns", gw2, []*extdns.Endpoint{ep2})
	require.Error(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			run: func(t *testing.T, _ *RecordStore, _ *DNSRecordService) {
				...
				hr2Row := row("u2")
				valid, err := svc.ValidateEndpointsByZone(ns, hr2, []*extdns.Endpoint{hr2Row.Endpoint})
				require.NoError(t, err)
...
func TestValidateEndpointsByZone_table(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rows, err := svc.ValidateEndpointsByZone(tt.ns, nil, tt.eps)
			if tt.errSub != "" {
				require.Error(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			run: func(t *testing.T, svc *DNSRecordService) {
				...
				groups := svc.ListRecordOwnerResource()
