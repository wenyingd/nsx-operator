...
func (s *BindingStore) Apply(i interface{}) error {
	...
	if binding.MarkedForDelete != nil && *binding.MarkedForDelete {
		...
		log.V(1).Info("delete SubnetConnectionBindingMap from store", "subnetConnectionBindingMap", binding)
		if err != nil {
			return err
		}
	} else {
		err := s.Add(binding)
		log.V(1).Info("add SubnetConnectionBindingMap to store", "subnetConnectionBindingMap", binding)
		if err != nil {
			return err
		}
	}
