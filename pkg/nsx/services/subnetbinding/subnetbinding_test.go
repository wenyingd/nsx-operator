...
func TestGetCRNameBySubnetConnectionBindingMap(t *testing.T) {
	svc := mockService()
	crName := svc.GetCRNameBySubnetConnectionBindingMap(bindingMap1)
	require.Equal(t, "binding1", crName)
	crName = svc.GetCRNameBySubnetConnectionBindingMap(nil)
	require.Equal(t, "", crName)
	crName = svc.GetCRNameBySubnetConnectionBindingMap(&model.SubnetConnectionBindingMap{
		Id:             String(bm1ID),
		DisplayName:    String("binding1"),
		SubnetPath:     String(parentSubnetPath1),
		VlanTrafficTag: Int64(201)})
...
func TestDeleteSubnetConnectionBindingMaps(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			name: "test with DeleteSubnetConnectionBindingMapsByCRUID",
			deleteFn: func(svc *BindingService) error {
				mockOrgRootClient.EXPECT().Patch(gomock.Any(), &enforceRevisionCheckParam).Return(nil)
				return svc.DeleteSubnetConnectionBindingMapsByCRUID(string(binding1.UID))
			},
			expErr:                "",
			expBindingMapsInStore: []*model.SubnetConnectionBindingMap{},
		}, {
			name: "test with non-existing BindingMaps by CR UID",
			deleteFn: func(svc *BindingService) error {
				return svc.DeleteSubnetConnectionBindingMapsByCRUID("invalid-uid")
			},
			expErr:                "",
			expBindingMapsInStore: []*model.SubnetConnectionBindingMap{createdBM1},
		}, {
			name: "test with DeleteSubnetConnectionBindingMapsByParentSubnets",
			deleteFn: func(svc *BindingService) error {
				mockOrgRootClient.EXPECT().Patch(gomock.Any(), &enforceRevisionCheckParam).Return(nil)
				return svc.DeleteSubnetConnectionBindingMapsByParentSubnets([]*model.VpcSubnet{parentSubnet1})
			},
			expErr:                "",
			expBindingMapsInStore: []*model.SubnetConnectionBindingMap{},
		}, {
