...
func (r *GatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	log.V(2).Info("Reconcile started", "Gateway", req.NamespacedName)
	...
	log.V(2).Info("Gateway loaded", "Gateway", req.NamespacedName, "class", gw.Spec.GatewayClassName,
	...
	if !shouldProcessGateway(gw) {
		...
		log.V(2).Info("Gateway reconcile exit: not managed, delete DNS", "Gateway", req.NamespacedName)
		return r.deleteAllDNSRecords(ctx, gw, req)
	}

	if !gw.DeletionTimestamp.IsZero() {
		log.Info("Reconciling Gateway delete", "Gateway", req.NamespacedName)
		log.V(2).Info("Gateway reconcile exit: Gateway deleting, delete DNS", "Gateway", req.NamespacedName)
		return r.deleteAllDNSRecords(ctx, gw, req)
	}

	if !hasUsableGatewayIP(gw) {
		log.Info("Gateway has no valid address, DNS records should be deleted", "Gateway", req.NamespacedName)
		log.V(2).Info("Gateway reconcile exit: no usable Gateway IP, delete DNS", "Gateway", req.NamespacedName)
		return r.deleteAllDNSRecords(ctx, gw, req)
	}
	...
	for _, rec := range desiredDNSRecords {
		...
		log.V(2).Info("DNS record upsert", "Gateway", req.NamespacedName, "ownerKind", rec.Owner.Kind,
...
func (r *GatewayReconciler) CollectGarbage(ctx context.Context) error {
	...
	log.V(2).Info("Gateway CollectGarbage started", "cachedGateways", len(cachedGatewaySet), "listenerSetEnabled", r.listenerSetEnabled)
...
func (r *GatewayReconciler) StartController(mgr ctrl.Manager, _ webhook.Server) error {
	...
	r.crdReady = true
	log.V(2).Info("Gateway StartController: Gateway API present", "listenerSetEnabled", listenerSetExists)
