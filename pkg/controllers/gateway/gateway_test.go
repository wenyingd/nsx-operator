...
func TestBuildRouteDNSEndpointsForAggregation_table(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			eps, err := gr.buildRouteDNSEndpointsForAggregation(ns, owner, refs, ps, tc.meta, tc.specHosts)
...
func TestPredicateNetworkInfoAllowedDNSDomainsChanged_table(t *testing.T) {
	pred := predicateNetworkInfoAllowedDNSDomainsChanged()
	niEmpty := &v1alpha1.NetworkInfo{ObjectMeta: metav1.ObjectMeta{Name: "n", Namespace: "ns"}, AllowedDNSDomains: nil}
	niWithDomains := niEmpty.DeepCopy()
	niWithDomains.AllowedDNSDomains = []string{"a.example.com"}
	niOld := niWithDomains.DeepCopy()
	niNew := niWithDomains.DeepCopy()
	niNew.AllowedDNSDomains = []string{"b.example.com"}
	niSame := niWithDomains.DeepCopy()
	...
	tests := []struct {
		...
		{"create_with_domains", true, func() bool { return pred.Create(event.CreateEvent{Object: niWithDomains}) }},
		{"create_empty_domains", false, func() bool { return pred.Create(event.CreateEvent{Object: niEmpty}) }},
		{"create_wrong_type", false, func() bool { return pred.Create(event.CreateEvent{Object: &corev1.Pod{}}) }},
		{"update_domains_change", true, func() bool { return pred.Update(event.UpdateEvent{ObjectOld: niOld, ObjectNew: niNew}) }},
		{"update_domains_unchanged", false, func() bool { return pred.Update(event.UpdateEvent{ObjectOld: niSame, ObjectNew: niSame.DeepCopy()}) }},
		{"update_wrong_type", false, func() bool {
			return pred.Update(event.UpdateEvent{ObjectOld: niOld, ObjectNew: &corev1.Pod{}})
		}},
		{"delete", false, func() bool { return pred.Delete(event.DeleteEvent{Object: niWithDomains}) }},
		{"generic", false, func() bool { return pred.Generic(event.GenericEvent{Object: niWithDomains}) }},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.Equal(t, tt.want, tt.f())
...
func TestNetworkInfoDNS_enqueueRequests_table(t *testing.T) {
	ctx := context.Background()
	ns := "app"
	nsObj := testNamespaceAllowedDNSZones(ns, map[string]string{"example.com": "/zones/t"})
	ni := &v1alpha1.NetworkInfo{ObjectMeta: metav1.ObjectMeta{Namespace: ns, Name: ns}}
	hr := testHTTPRouteAccepted(ns, "hr1", "u1", "x.example.com", ns, "gw1")
	grpcR := testGRPCRouteAccepted(ns, "gr1", "u1", "x.example.com", ns, "gw1")
	tlsR := &gatewayv1.TLSRoute{
		ObjectMeta: metav1.ObjectMeta{Namespace: ns, Name: "tr1", UID: "u-tls"},
	}
	gwHostname := testGatewayManaged(ns, "gw1", "*.example.com", "10.0.0.1", "")
	if gwHostname.Annotations == nil {
		gwHostname.Annotations = map[string]string{}
	}
	gwHostname.Annotations[servicecommon.AnnotationDNSHostnameKey] = "gw.example.com"
	gwNoHostname := testGatewayManaged(ns, "gw2", "*.example.com", "10.0.0.2", "")

	gr := newTestGatewayReconcilerFromObjects(t, apiResourcesForTests(false, true, true, true), nsObj, hr, grpcR, tlsR, gwHostname, gwNoHostname)

	t.Run("http_routes_in_namespace", func(t *testing.T) {
		got := networkInfoToRouteDNSRequests[gatewayv1.HTTPRoute, *gatewayv1.HTTPRoute](
			ctx, gr.Client, ni, dns.ResourceKindHTTPRoute, &gatewayv1.HTTPRouteList{})
		require.Len(t, got, 1)
		require.Equal(t, types.NamespacedName{Namespace: ns, Name: "hr1"}, got[0].NamespacedName)
	})
	t.Run("grpc_routes_in_namespace", func(t *testing.T) {
		got := networkInfoToRouteDNSRequests[gatewayv1.GRPCRoute, *gatewayv1.GRPCRoute](
			ctx, gr.Client, ni, dns.ResourceKindGRPCRoute, &gatewayv1.GRPCRouteList{})
		require.Len(t, got, 1)
		require.Equal(t, types.NamespacedName{Namespace: ns, Name: "gr1"}, got[0].NamespacedName)
	})
	t.Run("tls_routes_in_namespace", func(t *testing.T) {
		got := networkInfoToRouteDNSRequests[gatewayv1.TLSRoute, *gatewayv1.TLSRoute](
			ctx, gr.Client, ni, dns.ResourceKindTLSRoute, &gatewayv1.TLSRouteList{})
		require.Len(t, got, 1)
		require.Equal(t, types.NamespacedName{Namespace: ns, Name: "tr1"}, got[0].NamespacedName)
	})
	t.Run("gateway_with_hostname_anno_only", func(t *testing.T) {
		got := gr.networkInfoToGatewayDNSRequests(ctx, ni)
		require.Len(t, got, 1)
		require.Equal(t, types.NamespacedName{Namespace: ns, Name: "gw1"}, got[0].NamespacedName)
	})
	t.Run("non_networkinfo_returns_empty_for_routes", func(t *testing.T) {
		require.Empty(t, networkInfoToRouteDNSRequests[gatewayv1.HTTPRoute, *gatewayv1.HTTPRoute](
			ctx, gr.Client, hr, dns.ResourceKindHTTPRoute, &gatewayv1.HTTPRouteList{}))
	})
}
...
func TestGc_httpRouteDisabled_listOwnersAndPruneStore(t *testing.T) {
	...
	_, err = svc.CreateOrUpdateRecords(ctx, dns.NewOwnerScopedAggregatedRouteDNS(hrOwner, []dns.EndpointRow{*row}))
	require.NoError(t, err)
...
func TestCollectGarbage_deletesStaleGatewayIndex(t *testing.T) {
	...
	_, err := svc.CreateOrUpdateRecords(ctx, dns.NewOwnerScopedAggregatedRouteDNS(staleOwner, []dns.EndpointRow{*staleRow}))
	require.NoError(t, err)
...
func runReconcileRouteDNSCase(ctx context.Context, t *testing.T, br reconcileRouteDNSCase) {
	...
	case reconcileRouteDNSSuccess:
		...
		fc := fakeClientForGatewayTests(nsObj, gw, hr)
		gr := newTestGatewayReconcilerWithService(t, fc, ar, dnsTestServiceWithClient(fc))
		gr.ipCache = NewGatewayIPCache()
		gr.ipCacheWarmedOnStartup.Store(true)
		refreshIPCache(ctx, t, gr, gw)
