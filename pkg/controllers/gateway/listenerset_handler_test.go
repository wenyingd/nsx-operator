...
func Test_predicateFuncsListenerSet(t *testing.T) {
	...
	baseObjs := []client.Object{
		...
		newTestGateway("gw1", "ns1", "10.0.0.1", false, nil),
		newTestGateway("gw-a", "ns1", "10.0.0.2", false, nil),
		newTestGateway("gw-b", "ns1", "10.0.0.3", false, nil),
	}
	predR := func() *GatewayReconciler {
		fc := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(baseObjs...).
			WithStatusSubresource(&gatewayv1.Gateway{}).
			Build()
		return &GatewayReconciler{Client: fc}
	}
	...
	for _, tt := range []struct {
		...
		{"Create/with hostname", true, func() bool {
			return predR().predicateFuncsListenerSet().Create(event.CreateEvent{Object: newTestListenerSet("ls1", "ns1", "gw1", "h.example.com")})
		}},
		{"Create/no hostname", false, func() bool {
			ls := newTestListenerSet("ls1", "ns1", "gw1", "h.example.com")
			ls.Spec.Listeners[0].Hostname = nil
			return predR().predicateFuncsListenerSet().Create(event.CreateEvent{Object: ls})
		}},
		{"Update/parent changed", true, func() bool {
			...
			return predR().predicateFuncsListenerSet().Update(event.UpdateEvent{ObjectOld: o, ObjectNew: n})
		}},
		{"Update/hostname changed", true, func() bool {
			return predR().predicateFuncsListenerSet().Update(event.UpdateEvent{
		...
		{"Update/hostname order only", false, func() bool {
			return predR().predicateFuncsListenerSet().Update(event.UpdateEvent{ObjectOld: oldMulti, ObjectNew: newOrder})
		}},
		{"Update/no-op", false, func() bool {
			ls := newTestListenerSet("ls1", "ns1", "gw1", "h.example.com")
			return predR().predicateFuncsListenerSet().Update(event.UpdateEvent{ObjectOld: ls, ObjectNew: ls.DeepCopy()})
		}},
	} {
...
func Test_listenerSetEnqueueHandler_Update_parentRef(t *testing.T) {
