...
var PredicateFuncsSubnets = predicate.Funcs{
	...
	UpdateFunc: func(e event.UpdateEvent) bool {
		oldObj := e.ObjectOld.(*v1alpha1.Subnet)
		newObj := e.ObjectNew.(*v1alpha1.Subnet)
...
var PredicateFuncsSubnetSets = predicate.Funcs{
	...
	UpdateFunc: func(e event.UpdateEvent) bool {
		oldObj := e.ObjectOld.(*v1alpha1.SubnetSet)
		newObj := e.ObjectNew.(*v1alpha1.SubnetSet)
...
func requeueBindingMapsBySubnetDelete(ctx context.Context, c client.Client, obj client.Object, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
...
func requeueSubnetConnectionBindingMapsBySubnet(ctx context.Context, c client.Client, namespace string, subnet string, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	...
	for _, bm := range bindingMapList.Items {
		if bm.Spec.SubnetName == subnet || bm.Spec.TargetSubnetName == subnet {
			log.Info("Requeue SubnetConnectionBindingMap because because the dependent Subnet is ready", "Namespace", namespace, "Name", bm.Name)
...
func requeueBindingMapsBySubnetSetDelete(ctx context.Context, c client.Client, obj client.Object, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
...
func requeueSubnetConnectionBindingMapsBySubnetSet(ctx context.Context, c client.Client, namespace string, subnetSet string, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	...
	err := c.List(ctx, bindingMapList, client.InNamespace(namespace))
	if err != nil {
		log.Error(err, "Failed to list SubnetConnectionBindingMaps with SubnetSet event", "Namespace", namespace, "Subnet", subnetSet)
		return
	}
	for _, bm := range bindingMapList.Items {
		if bm.Spec.TargetSubnetSetName == subnetSet {
			log.Info("Requeue SubnetConnectionBindingMap because because the dependent SubnetSet is ready or updated", "Namespace", namespace, "Name", bm.Name)
			q.Add(reconcile.Request{
				NamespacedName: types.NamespacedName{
					Name:      bm.Name,
					Namespace: bm.Namespace,
