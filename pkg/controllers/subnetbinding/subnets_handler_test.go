...
func TestPredicateFuncsSubnets(t *testing.T) {
	...
	assert.False(t, PredicateFuncsSubnets.CreateFunc(createEvent))
	assert.False(t, PredicateFuncsSubnets.Update(updateEvent1))
	assert.False(t, PredicateFuncsSubnets.Update(updateEvent2))
	assert.True(t, PredicateFuncsSubnets.Update(updateEvent3))
	assert.False(t, PredicateFuncsSubnets.Delete(deleteEvent))
	assert.False(t, PredicateFuncsSubnets.GenericFunc(genericEvent))
}
...
func TestPredicateFuncsSubnetSets(t *testing.T) {
	...
	deleteEvent := event.DeleteEvent{Object: net1}
	genericEvent := event.GenericEvent{Object: net1}
	assert.False(t, PredicateFuncsSubnetSets.CreateFunc(createEvent))
	assert.False(t, PredicateFuncsSubnetSets.Update(updateEvent1))
	assert.True(t, PredicateFuncsSubnetSets.Update(updateEvent2))
	assert.False(t, PredicateFuncsSubnetSets.Delete(deleteEvent))
	assert.False(t, PredicateFuncsSubnetSets.GenericFunc(genericEvent))
}
...
func TestRequeueSubnetConnectionBindingMapsBySubnet(t *testing.T) {
	...
	requeueBindingMapsBySubnetDelete(ctx, fakeClient, subnet1, myQueue)
...
func TestRequeueSubnetConnectionBindingMapsBySubnetSet(t *testing.T) {
	...
	requeueBindingMapsBySubnetSetDelete(ctx, fakeClient, subnetSet1, myQueue)
	require.Equal(t, 0, myQueue.Len())
	requeueBindingMapsBySubnetSetDelete(ctx, fakeClient, subnetSet2, myQueue)
	require.Equal(t, 1, myQueue.Len())
	item, _ := myQueue.Get()
	assert.Equal(t, req, item)
	myQueue.Done(item)
