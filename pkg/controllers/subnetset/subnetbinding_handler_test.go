...
var (
	...
	subnetSet1 = &v1alpha1.SubnetSet{
		ObjectMeta: metav1.ObjectMeta{Name: "parentSet", Namespace: "default", Finalizers: []string{servicecommon.SubnetSetFinalizerName}},
	}
...
func TestRequeueSubnetByBindingMap(t *testing.T) {
	...
	requeueSubnetSetByBindingMapUpdate(ctx, fakeClient, bm1, bm1, myQueue)
	require.Equal(t, 0, myQueue.Len())

	bm2.Status = v1alpha1.SubnetConnectionBindingMapStatus{
		Conditions: []v1alpha1.Condition{
			{
				Type:   v1alpha1.Ready,
				Status: corev1.ConditionTrue,
	...
	requeueSubnetSetByBindingMapUpdate(ctx, fakeClient, bm2, bm2, myQueue)
	require.Equal(t, 0, myQueue.Len())
	requeueSubnetSetByBindingMapDelete(ctx, fakeClient, bm2, myQueue)
	require.Equal(t, 1, myQueue.Len())
	item, _ := myQueue.Get()
	assert.Equal(t, req1, item)
	myQueue.Done(item)

	bm3.Status = v1alpha1.SubnetConnectionBindingMapStatus{
		Conditions: []v1alpha1.Condition{
			{
				Type:   v1alpha1.Ready,
				Status: corev1.ConditionTrue,
	...
	requeueSubnetSetByBindingMapUpdate(ctx, fakeClient, bm3, bm3, myQueue)
	require.Equal(t, 1, myQueue.Len())
	item, _ = myQueue.Get()
	assert.Equal(t, req2, item)
	myQueue.Done(item)
	requeueSubnetSetByBindingMapDelete(ctx, fakeClient, bm3, myQueue)
	require.Equal(t, 0, myQueue.Len())
}
