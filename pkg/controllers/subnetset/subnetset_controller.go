...
var (
	...
	msgFailAddFinalizer = "Failed to add finalizer on SubnetSet for the dependency by SubnetConnectionBindingMap %s"
	msgFailDelFinalizer = "Failed to remove finalizer on SubnetSet after no the dependency by SubnetConnectionBindingMaps"
	msgDeleteInUse      = "SubnetSet is used by SubnetConnectionBindingMap %s and not able to delete"
	reasonDeleteInUse   = "SubnetSetInUse"
)
...
func (r *SubnetSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	hasBindings := r.subnetSetHasBindings(string(subnetsetCR.UID))
	if len(hasBindings) > 0 {
		if !controllerutil.ContainsFinalizer(subnetsetCR, servicecommon.SubnetSetFinalizerName) {
			...
			if err := r.Client.Update(ctx, subnetsetCR); err != nil {
				log.Error(err, "Failed to add finalizer", "subnetSet", req.NamespacedName)
				r.StatusUpdater.UpdateFail(ctx, subnetsetCR, err, "Unable to add finalizer on SubnetSet used by SubnetConnectionBindingMap",
					setSubnetSetReadyStatusFalse, fmt.Sprintf(msgFailAddFinalizer, hasBindings[0].Name))
				return ResultRequeue, err
			}
		}
	} else {
		if controllerutil.ContainsFinalizer(subnetsetCR, servicecommon.SubnetSetFinalizerName) {
			...
			if err := r.Client.Update(ctx, subnetsetCR); err != nil {
				log.Error(err, "Failed to delete finalizer", "subnet", req.NamespacedName)
				r.StatusUpdater.UpdateFail(ctx, subnetsetCR, err, "Unable to remove finalizer from SubnetSet",
					setSubnetSetReadyStatusFalse, fmt.Sprintf(msgFailDelFinalizer))
				return ResultRequeue, err
			}
		}
	}
	...
	if !subnetsetCR.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if len(hasBindings) > 0 {
			err := fmt.Errorf("subnetSet %s is used by SubnetConnectionBindingMaps %s", req.String(), hasBindings[0].GetName())
			log.Error(err, "Failed to delete SubnetSet CR because it is used by SubnetConnectionBindingMaps, retrying", "Subnet", req.NamespacedName)
			r.StatusUpdater.DeleteFail(req.NamespacedName, nil, err)
			r.setSubnetDeletionFailedStatus(ctx, subnetsetCR, metav1.Now(), fmt.Sprintf(msgDeleteInUse, hasBindings[0].GetName()), reasonDeleteInUse)
			return ResultRequeue, err
		}
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			&common.EnqueueRequestForDependency{
				...
				RequeueByUpdate: requeueSubnetSetByBindingMapUpdate,
				RequeueByDelete: requeueSubnetSetByBindingMapDelete,
			},
...
func (r *SubnetSetReconciler) deleteSubnetForSubnetSet(subnetSet v1alpha1.SubnetSet, updateStatus, ignoreStaleSubnetPort bool) error {
	...
	// corresponding NSX Subnet. This happens in the GC case to scale-in the NSX Subnet if no subnet Port exists.
	...
	for _, nsxSubnet := range nsxSubnets {
		...
		func() {
			...
			if deleteBindingMaps {
				if err := r.BindingService.DeleteSubnetConnectionBindingMapsByParentSubnets([]*model.VpcSubnet{nsxSubnet}); err != nil {
					deleteErr := fmt.Errorf("failed to delete NSX SubnetConnectionBindingMaps referred to Subnet/%s: %+v", *nsxSubnet.Id, err)
