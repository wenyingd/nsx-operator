...
func TestSetNSNetworkReadyCondition(t *testing.T) {
	nsName := "test-ns"
	msg := NSMsgVPCCreateUpdateFailed
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			setNSNetworkReadyCondition(ctx, k8sClient, nsName, msg, msgErr)
		})
	}
}
...
func TestGetNSNetworkCondition(t *testing.T) {
	...
	require.True(t, nsConditionEquals(vpcNotReadyCondition, *NSMsgVPCCreateUpdateFailed.getNSNetworkCondition(msgErr)))
}
