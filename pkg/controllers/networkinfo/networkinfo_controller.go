...
var (
	NSMsgVPCNetCfgNotFound       = newNsUnReadyMessage("Error happened to get VPC network configuration %s: %v", NSReasonVPCNetConfigNotReady)
	...
	NSMsgVPCGwConnectionNotReady = newNsUnReadyMessage("System VPC gateway connection is not ready: %v", NSReasonVPCNetConfigNotReady)
	NSMsgVPCCreateUpdateFailed   = newNsUnReadyMessage("Error happened to create or update VPC: %v", NSReasonVPCNotReady)
	NSMsgVPCNsxLBSNotReady       = newNsUnReadyMessage("Error happened to get NSX LBS path in VPC: %v", NSReasonVPCNotReady)
	NSMsgVPCAviSubnetFailure     = newNsUnReadyMessage("Error happened to get Avi Load balancer Subnet info", NSReasonVPCNotReady)
	NSMsgVPCGetExtIPBlockFailed  = newNsUnReadyMessage("Error happened to get external IP blocks: %v", NSReasonVPCNotReady)
	NSMsgVPCNoExternalIPBlock    = newNsUnReadyMessage("System VPC has no external IP blocks", NSReasonVPCNotReady)
	NSMsgVPCAutoSNATDisabled     = newNsUnReadyMessage("System VPC has disabled auto SNAT", NSReasonVPCSnatNotReady)
	NSMsgVPCNonDefaultSNATIP     = newNsUnReadyMessage("Default SNAT IP is not allocated in VPC: %v", NSReasonVPCSnatNotReady)
...
func (r *NetworkInfoReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		...
		setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCNetCfgNotFound, ncName, err)
	...
	if !_exist {
		...
		log.Info(message)
		err = fmt.Errorf("VPCNetworkConfig %s not found", ncName)
		updateFail(r, ctx, networkInfoCR, &err, r.Client, nil)
		setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCNetCfgNotFound, ncName, err)
	...
	if err != nil {
		...
		updateFail(r, ctx, networkInfoCR, &err, r.Client, nil)
		setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgSystemVPCNetCfgNotFound, err)
		return common.ResultRequeueAfter10sec, err
	}
	...
	if !gatewayConnectionReady {
		...
		if ncName != commonservice.SystemVPCNetworkConfigurationName {
			...
			setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCGwConnectionNotReady, err)
	...
	if err != nil {
		...
		updateFail(r, ctx, networkInfoCR, &err, r.Client, nil)
		setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCCreateUpdateFailed, err)
		return common.ResultRequeueAfter10sec, err
	}
	...
	if isPreCreatedVPC {
		...
		if createdVpc.LoadBalancerVpcEndpoint == nil || createdVpc.LoadBalancerVpcEndpoint.Enabled == nil ||
			...
			if err != nil {
				...
				updateFail(r, ctx, networkInfoCR, &err, r.Client, nil)
				setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCNsxLBSNotReady, err)
				return common.ResultRequeueAfter10sec, err
			}
		}
	} else {
	...
	if err != nil {
		...
		updateFail(r, ctx, networkInfoCR, &err, r.Client, nil)
		setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCGetExtIPBlockFailed, err)
		return common.ResultRequeueAfter10sec, err
	}
	hasExternalIPs := true
	if ncName == commonservice.SystemVPCNetworkConfigurationName {
		...
		if !hasExternalIPs {
			...
			setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCNoExternalIPBlock, err)
			return common.ResultRequeueAfter60sec, nil
		}
	}
	...
	if autoSnatEnabled {
		...
		if err != nil {
			...
			setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCNonDefaultSNATIP, err)
			return common.ResultRequeueAfter10sec, err
		}
	}
	if ncName == commonservice.SystemVPCNetworkConfigurationName {
		...
		if !autoSnatEnabled {
			...
			setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCAutoSNATDisabled, err)
			return common.ResultRequeueAfter60sec, nil
		}
	}
	...
	if lbProvider == vpc.AVILB && createdVpc.LoadBalancerVpcEndpoint != nil && createdVpc.LoadBalancerVpcEndpoint.Enabled != nil && *createdVpc.LoadBalancerVpcEndpoint.Enabled {
		...
		if err != nil {
			...
			updateFail(r, ctx, networkInfoCR, &err, r.Client, state)
			setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCAviSubnetFailure, err)
			return common.ResultRequeueAfter10sec, err
		}
	}
	...
	setNSNetworkReadyCondition(ctx, r.Client, req.Namespace, NSMsgVPCIsReady)
