...
func TestUpdateSuccess(t *testing.T) {
	...
	t.Run("clear_dns_and_condition_error", func(t *testing.T) {
		...
		_, err := r.Reconcile(ctx, ctrl.Request{NamespacedName: types.NamespacedName{Namespace: "ns", Name: "lb-err-rec"}})
		assert.Error(t, err)
	})
}
...
func TestServiceLbReconciler_Reconcile_table(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := r.Reconcile(ctx, ctrl.Request{NamespacedName: tt.req})
			if tt.wantErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
