...
// stubValidatedRows mimics ValidateEndpointsByZone for *.example.com under /zones/t (same stub domain as dns.getDNSZones).
...
func TestBuildLoadBalancerServiceDNSBatch(t *testing.T) {
	...
	w := mockdns.NewMockDNSRecordProvider(mockCtl)
	assignDNSListStubs(w)
	w.EXPECT().ValidateEndpointsByZone(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
		func(_ string, _ *dns.ResourceRef, eps []*extdns.Endpoint) ([]dns.EndpointRow, error) {
...
func TestServiceLbReconciler_reconcileLoadBalancerServiceDNS(t *testing.T) {
	...
	var gotBatch *dns.AggregatedDNSEndponts
	m.EXPECT().CreateOrUpdateRecords(gomock.Any(), gomock.Any()).Do(func(_ context.Context, b *dns.AggregatedDNSEndponts) {
		gotBatch = b
	}).Return(true, nil).Times(1)
	r := &ServiceLbReconciler{
		Client: serviceLbFakeClient(scheme, false, svc),
		DNS:    m,
	}
