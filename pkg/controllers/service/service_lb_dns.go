...
func buildLoadBalancerServiceDNSBatch(svc *v1.Service, w dns.DNSRecordProvider) (*dns.AggregatedDNSEndponts, error) {
	...
	rows, err := w.ValidateEndpointsByZone(svc.Namespace, owner, eps)
	if err != nil {
		return nil, err
	}
	if len(rows) == 0 {
...
func (r *ServiceLbReconciler) reconcileLoadBalancerServiceDNS(ctx context.Context, svc *v1.Service) (published bool, err error) {
	...
	if batch == nil || len(batch.Rows) == 0 {
		_, err = r.DNS.DeleteRecordByOwnerNN(ctx, dns.ResourceKindService, svc.Namespace, svc.Name)
		return false, err
	}
	_, err = r.DNS.CreateOrUpdateRecords(ctx, batch)
	if err != nil {
		return false, err
	}
	return true, nil
...
func (r *ServiceLbReconciler) enqueueLBServiceRequestsFromNetworkInfo(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	if err := r.Client.List(ctx, svcList, client.InNamespace(ni.Namespace)); err != nil {
